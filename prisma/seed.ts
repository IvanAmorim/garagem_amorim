...
async function main() {
  ...
  // Admin user
  const hashedPassword = await bcrypt.hash("wuffeJ-femfam-remho4", 12)
  const admin = await db.user.upsert({
    where: { email: "pedrosamorim601@gmail.com" },
    update: {},
    create: {
      name: "Pedro",
      email: "pedrosamorim601@gmail.com",
      password: hashedPassword,
      role: "MECHANIC",
    },
  })

  const hashedPassword2 = await bcrypt.hash("tavcaX-dyfveb-5zunju", 12)
  const admin2 = await db.user.upsert({
    where: { email: "ivanamorim65@gmail.com" },
    update: {},
    create: {
      name: "Pedro",
      email: "ivanamorim65@gmail.com",
      password: hashedPassword,
      role: "ADMIN",
    },
  })
  ...
  const settings = await db.workshopSettings.upsert({
    ...
    create: {
      id: "default",
      name: "Garagem Amorim",
      address: "Rio Frio, Arcos de Valdevez",
      nif: "999999999",
      phone: "+351 911 510 848",
      email: "pedrosamorim601@gmail.com",
      laborHourRate: 20,
      defaultTaxRate: 23,
      quotePrefix: "ORC",
